...
DOCUMENTATION = r"""
    ...
    author:
        - Matthew Howle <mdhowle@gmail.com>
    short_description: Active Directory dynamic inventory plugin
    requirements:
        - python >= 2.7
        - ldap3
    optional:
    ...
    description:
        ...
        - If no username is defined, Kerberos will be used to connect to the server.
    options:
        plugin:
            description: Marks this as an instance of the 'ad' plugin.
            required: true
            choices: ['mdhowle.ad.ad_inventory']
        server:
        ...
        starttls:
            ...
            default: true
        base:
            description: Starting distinguished name for the search. If unspecified, the default naming context will be used.
            required: false
            type: str
            default: null
        scope:
        ...
        username:
            description: Username to bind as. It can be the distinguished name of the user, or "SHORTDOMAIN\user".
              If unspecified, Kerberos authentication will be used.
            required: false
            type: str
            default: null
        password:
...
class InventoryModule(BaseInventoryPlugin):
    ...
    def _create_client(self):
        ...
        for server in servers:
            ...
            if base is None:
                result = self._connection.search(search_base="", search_filter="(dnsHostName=%s)" % server,
                                                 search_scope=BASE, attributes=["defaultNamingContext"])
                if result:
                    base = self._connection.server.info.other["defaultNamingContext"][0]
    ...
    def _find_closest_dc(self):
        ...
        if ldap_servers:
            ...
            results = pool.map(ldap_ping, [(str(server.target)[:-1], cargs) for server in ldap_servers])
            pool.close()
